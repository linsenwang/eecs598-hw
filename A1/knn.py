...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i, :] = torch.sum((x_train[i] - x_test), dim=tuple(range(1, x_train.dim()))) ** 2
    #57.3X speedup
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # dists = torch.sum((x_train.unsqueeze(1) - x_test.unsqueeze(0)) ** 2, dim=tuple(range(1, x_train.dim())))

    # 展平数据 (num_train, D) 和 (num_test, D)
    x_train_flat = x_train.to('cuda').view(num_train, -1)
    x_test_flat = x_test.to('cuda').view(num_test, -1)

    # 计算平方范数
    x_train_sq = (x_train_flat ** 2).sum(dim=1, keepdim=True)  # (num_train, 1)
    x_test_sq = (x_test_flat ** 2).sum(dim=1, keepdim=True).T  # (1, num_test)

    # 计算点积
    dot_product = x_train_flat @ x_test_flat.T  # (num_train, num_test)

    # 计算平方欧几里得距离
    dists = x_train_sq + x_test_sq - 2 * dot_product
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        values, indices = torch.topk(dists[:, i], k, largest=False)
                # 使用字典统计出现次数
        count_dict = {}
        for item in list(y_train[indices].tolist()):
        ...
        # 找出所有出现最大次数的项
        most_common_items = [key for key, value in count_dict.items() if value == max_count]

        # 返回最小的项
        most_common_item = min(most_common_items)
        # y_pred[i] = y_train[most_common_item]
        y_pred[i] = most_common_item
