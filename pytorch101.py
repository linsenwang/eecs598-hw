...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_multiple = (start + 9) // 10 * 10
    stop_multiple = (stop // 10) * 10

    if start_multiple > stop_multiple:
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.zeros(3, 8, dtype=torch.int8)
    y[:3, :4] = x.reshape(-1, 4)[:3, :]
    y[:3, 4:] = x.reshape(-1, 4)[3:, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    z = torch.zeros(batch_size, x.shape[1], y.shape[2])
